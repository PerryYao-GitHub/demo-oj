<template>
  <div class="admin-update-question-page">
...
import type { QuestionVO, QuestionRequest, JudgeCase } from '../types/question'
...
const fetchQuestion = async () => {
  try {
    const response = await request.get<AppResponse<QuestionVO>>(`/api/question/${questionId}`)
    if (response.data.code === 0) {
      const question = response.data.data
      title.value = question.title
      description.value = question.description
      selectedTags.value = question.tags
      timeLimit.value = question.judgeConfig?.timeLimit || null
      memoryLimit.value = question.judgeConfig?.memoryLimit || null

      // 从 judgeCase 反解出 pairs
      const judgeCase = question.judgeCase
      if (judgeCase) {
        pairs.value = judgeCase.inputs.map((input, index) => ({
          input,
          output: judgeCase.outputs[index] || ''
        }))
      }
    } else {
      alert(`Failed to fetch question: ${response.data.message}`)
    }
  } catch (error) {
...
const handleSubmit = async () => {
  ...
  try {
    const response = await request.post<AppResponse<null>>('/api/admin/update/question', payload)
    if (response.data.code === 0) {
      alert('Question updated successfully!')
    } else {
      alert(`Failed to update question: ${response.data.message}`)
    }
  } catch (error) {
